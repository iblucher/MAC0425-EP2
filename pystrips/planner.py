...
class ProgressionPlanning(object):
    ...
    def applicable(self, state):
        ...
        applicable_actions = []
        st = list(state)
        for actions in self._all_actions:
            for action in actions.precond:
                if action in st:
    ...
    def successor(self, state, action):
        ...
        positive = action.pos_effect
        negative = action.neg_effect
        diff = positive.difference(negative)
        successor_state = state.union(diff)
        return successor_state

    def goal_test(self, state):
        ''' Return true if `state` is a goal state. '''
        for goal in self._problem.goal:
            goal = set(goal)
